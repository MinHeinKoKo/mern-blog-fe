...
const CropEasy = ({ photo, setOpenCrop }) => {
  ...
  const handleCropComplete = (cropedArea, cropedAreaPixedls) => {
    setCroppAreaPixels(cropedAreaPixedls);
  };
  ...
  const { mutate, isLoading } = useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast.success("Profile photo has uploaded");
    },
    onError: (error) => {
  ...
  const handleCropImage = async () => {
    try {
      const croppedImg = await getCroppedImg(photo?.url, croppAreaPixels);
      ...
      <div className="bg-white h-fit w-full sm:max-w-[350px] p-5 rounded-lg">
        ...
        <div className="flex justify-between gap-2 flex-wrap">
          ...
          disabled={isLoading}
            ...
            className="px-5 py-2.5 rounded-lg  bg-blue-500 text-white text-sm disabled:opacity-70"
          >
            Crop & Upload
          </button>
        </div>
